...
export default {
    ...
    methods: {
        getData() {
            if (this.$store.state.recruitId == '') {
                this.$message.warning('请在列表中重新选择需要编辑的岗位！');
                this.$router.push('/recruit/list');
            } else {
                ajaxUrl.getEmploymentDetail(this.$store.state.recruitId).then(res => {
        ...
        submit() {
            ajaxUrl.updateEmployment(this.form).then(res => {
