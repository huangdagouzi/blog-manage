<template>
    <div>
        <div class="container">
            ...
            <el-upload
                class="upload-pic"
                action="http://www.aitradeapp.com/api/static/upload/"
...
export default {
                    ...
                    toolbar: {
                        ...
                        handlers: {
                            image: function(value) {
                                if (value) {
                                    // 调用图片上传
                                    document.querySelectorAll('.el-upload')[1].click();
                                } else {
    ...
    watch: {
        $route(to, from) {
            ...
            if (to.fullPath.search('/news/edit') != -1) {
                this.getData();
            } else {
            }
        }
    ...
    methods: {
        getData() {
            if (this.$store.state.newsId == '') {
                this.$message.warning('请在列表中重新选择需要编辑的新闻！');
                this.$router.push('/news/list');
            } else {
                ajaxUrl.getBlogDetail(`id=${this.$store.state.newsId}`).then(res => {
                    this.form = res.data.data;
                    this.form.publishTime = this.form.publishTime;
        ...
        submit() {
            ...
            ajaxUrl.updateBlog(params).then(res => {
